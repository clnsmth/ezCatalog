...
var PASTA_CONFIG = {
   "server": "https://pasta-s.lternet.edu/package/search/eml?", // PASTA server
...
function buildHtml(citations) {
   ...
   for (var i = 0; i < citationCount; i++) {
      ...
      var link = (citation["doi"]) ? citation["doi"].slice(0, -1) : "https://portal-s.edirepository.org/nis/mapbrowse?packageid=" + citation["pid"];
...
function buildCitationsFromPasta(pastaDocs) {
   ...
   for (var i = 0; i < pastaDocs.length; i++) {
      ...
      try {
         ...
         link = ("https://portal-s.edirepository.org/nis/mapbrowse?packageid=" +
